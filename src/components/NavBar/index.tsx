...
const NavBar = () => {
  ...
  return (
    <div className="flex justify-between  p-8 container mx-auto">
      ...
      <div className="hidden md:flex items-center gap-10 container mx-auto">
        {NavItems.map((item) => (
      ...
      <div className="md:hidden">
        <button onClick={() => setIsMenuOpen(!isMenuOpen)}>
          ...
          <div className={`absolute top-[100px] left-0 w-screen h-[calc(100vh-100px)] flex flex-col items-center justify-center gap-10  transition duration-500 ` + (isMenuOpen ? 'translate-x-0' : 'translate-x-full')}>
            {NavItems.map((item) => (
              <a key={item} className="text-md text-gray-500 hover:border-b-2 border-blue-500 hover:text-blue-500 "  href={`#${item.toLowerCase()}`}>
                {item}
              </a>
